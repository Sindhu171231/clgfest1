...
const RegisterPage = () => {
    ...
    const { register, requestOTP, user } = useContext(AuthContext);
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            await register(name, email, phone, password, role, branch, college, null); // Removed OTP parameter
            window.showToast('success', 'Registration successful!');
            navigate('/login');
        } catch (error) {
            window.showToast('error', error.response?.data?.message || 'Registration failed');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 p-4">
            <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md transform transition-all duration-300 hover:scale-[1.02]">
                <div className="text-center mb-8">
                    ...
                    <h2 className="text-3xl font-extrabold text-gray-800">Join COLORIDO 2K25</h2>
                ...
                <p className="mt-6 text-center text-sm text-gray-600">
                    Already have an account? <Link to="/login" className="text-indigo-600 hover:underline font-bold">Login</Link>
                </p>
            </div>
        </div>
    );
